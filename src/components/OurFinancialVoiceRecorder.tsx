...
interface OurFinancialVoiceRecorderProps {
  onFieldsExtracted: (fields: OurFinancialData) => void;
}

const OurFinancialVoiceRecorder = ({ onFieldsExtracted }: OurFinancialVoiceRecorderProps) => {
  ...
  const processAudio = async (audioBlob: Blob) => {
    try {
      ...
      // Step 2: Extract financial fields from text (using our specific function)
      const fieldsResponse = await supabase.functions.invoke('extract-our-financial-fields', {
        body: { text }
      });
      ...
      // Map the extracted data to our structure (data already in correct format)
      const ourFinancialData: OurFinancialData = {
        payment_for: extractedFields.payment_for || '',
        transaction_type: extractedFields.transaction_type,
        who_paid: extractedFields.who_paid || 'Both',
        for_who: extractedFields.for_who || 'Both',
        amount: extractedFields.amount,
        currency: extractedFields.currency,
        transaction_date: extractedFields.transaction_date,
        description: extractedFields.description
      };
      
      // Apply extracted fields
      onFieldsExtracted(ourFinancialData);
        ...
        <div className="flex flex-col items-center space-y-6">
          <div className="text-center">
            <p className="text-sm text-muted-foreground mb-6">
              Record details about your personal financial transaction
            </p>
          </div>
          ...
          <div className="bg-muted/50 rounded-lg p-8 text-center">
            ...
            <p className="text-xs text-muted-foreground italic">
              "I paid $50 to Costco for groceries today" or
              "Raiana paid 200 toman for Uber for both of us"
            </p>
          </div>
        </div>
      </CardContent>
...
export default OurFinancialVoiceRecorder;